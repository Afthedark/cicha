...
import type { PhotoAlbum, GalleryPhoto, Category } from '../../types';
...
export const AdminGalleryPage: React.FC = () => {
  ...
  const fetchGalleryCategories = async () => {
    setLoadingCats(true);
    try {
      const data = await adminApi.getCategories('gallery');
      if (data && data.length > 0) {
        setGalleryCategories(data);
      } else {
        const defaults: Category[] = [
          { id: 1, name: 'Institucional', slug: 'institucional', type: 'gallery' },
          { id: 2, name: 'Eventos Bilaterales', slug: 'eventos-bilaterales', type: 'gallery' },
          { id: 3, name: 'Rondas de Negocios', slug: 'rondas-de-negocios', type: 'gallery' },
          { id: 4, name: 'Visitas Diplomáticas', slug: 'visitas-diplomaticas', type: 'gallery' },
          { id: 5, name: 'Comunidad & Cultura', slug: 'comunidad-cultura', type: 'gallery' },
        ];
        setGalleryCategories(defaults);
      }
    } catch {
  ...
  const handleCreateCategory = async (e: React.FormEvent) => {
    ...
    setCatError(null);
    try {
      await adminApi.createCategory({ name: catNameInput.trim(), type: 'gallery' });
      setCatNameInput('');
      await fetchGalleryCategories();
      await fetchAlbums();
    } catch (err: any) {
      setCatError(err.response?.data?.message || 'Error al crear la categoría');
    } finally {
  ...
  const handleUpdateCategory = async (e: React.FormEvent) => {
    ...
    setCatError(null);
    try {
      await adminApi.updateCategory(editingCat.id, { name: catNameInput.trim(), type: 'gallery' });
      setEditingCat(null);
      setCatNameInput('');
      await fetchGalleryCategories();
      await fetchAlbums();
    } catch (err: any) {
      setCatError(err.response?.data?.message || 'Error al actualizar la categoría');
    } finally {
      setCatActionLoading(false);
    }
  };
  ...
  const handleDeleteCategory = async (cat: Category) => {
    if (!window.confirm(`¿Está seguro de eliminar la categoría "${cat.name}"?`)) return;
    setCatActionLoading(true);
    setCatError(null);
    try {
      ...
      setCatError(err.response?.data?.message || 'Error al eliminar la categoría');
    } finally {
      setCatActionLoading(false);
    }
  };
  ...
  const handleOpenCreateAlbum = () => {
    ...
    const defaultCat = galleryCategories.length > 0 ? galleryCategories[0].name : 'Institucional';
    setAlbumForm({
      title: '',
      category: defaultCat,
      event_date: new Date().toISOString().slice(0, 10),
  ...
  const filterCategories = Array.from(
    new Set([
      ...
      ...albums.map((a) => a.category).filter(Boolean),
    ])
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white rounded-2xl p-6 border border-slate-200 shadow-sm">
        ...
        <div className="flex items-center gap-2 self-start sm:self-auto flex-wrap">
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border border-slate-300 bg-white hover:bg-slate-50 text-slate-700 font-bold text-xs shadow-xs transition-all cursor-pointer"
          >
            <Tag className="w-4 h-4 text-amber-600" />
            <span>Gestionar Categorías ({galleryCategories.length})</span>
          </button>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <select
                ...
                className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs bg-white focus:outline-hidden focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 cursor-pointer"
              >
                {filterCategories.map((cat) => (
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-200 pb-3">
              ...
              {albumForm.photos.length > 0 && (
                <button
                  ...
                  className="px-2.5 py-1 rounded-lg text-rose-600 hover:bg-rose-100/70 text-[11px] font-bold flex items-center gap-1 transition-colors self-start sm:self-auto cursor-pointer"
                >
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          ...
                          className="absolute top-1 right-1 w-6 h-6 rounded-full bg-rose-600/90 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 hover:bg-rose-700 shadow-md transition-opacity cursor-pointer"
                        ...
                        <button
                          ...
                          className={`absolute bottom-1 left-1 px-2 py-0.5 rounded text-[9px] font-bold shadow-xs transition-all flex items-center gap-1 cursor-pointer ${
                            isCover
                              ? 'bg-blue-600 text-white'
      ...
      <Modal
        ...
        title="Gestión de Categorías de la Galería de Fotos"
        maxWidth="lg"
      >
        <div className="space-y-5 text-xs">
          <p className="text-slate-600">
            Cree, edite o elimine las categorías temáticas utilizadas para clasificar los álbumes y eventos en fotos.
          </p>
          ...
          >
            <input
              type="text"
              placeholder="Nombre de categoría (ej. Misiones Comerciales)..."
              value={catNameInput}
              onChange={(e) => setCatNameInput(e.target.value)}
              className="flex-1 px-3 py-2 bg-white border border-slate-200 rounded-lg text-xs focus:outline-hidden focus:border-blue-500"
              required
            />
            {editingCat ? (
              <>
                <button
                  ...
                  className="px-3.5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-lg transition-colors cursor-pointer"
                >
                  {catActionLoading ? 'Guardando...' : 'Guardar'}
                </button>
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="px-3 py-2 bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold rounded-lg transition-colors cursor-pointer"
                >
                  Cancelar
                </button>
              </>
              ...
              >
                ...
                <span>{catActionLoading ? 'Creando...' : 'Agregar'}</span>
              </button>
            )}
          </form>
          ...
          <div className="space-y-2">
            <h4 className="font-bold text-slate-700 text-xs uppercase tracking-wider">
              Categorías Registradas ({galleryCategories.length})
            </h4>

            {loadingCats ? (
              <div className="py-6 flex justify-center">
                <Loader size="sm" text="Cargando categorías..." />
              </div>
            ) : galleryCategories.length === 0 ? (
              <p className="text-slate-400 italic py-3 text-center">No hay categorías registradas.</p>
            ) : (
              <div className="max-h-60 overflow-y-auto space-y-1.5 pr-1">
                {galleryCategories.map((cat) => {
                  const albumCount = albums.filter((a) => a.category === cat.name).length;
                  return (
                    <div
                      key={cat.id}
