...
export const AdminMembersPage: React.FC = () => {
  ...
  const toggleMemberSector = (sectorName: string) => {
    const currentSectors = memberForm.sector
      ? memberForm.sector.split(',').map((s) => s.trim()).filter(Boolean)
      : [];
    
    let updated: string[];
    if (currentSectors.includes(sectorName)) {
      updated = currentSectors.filter((s) => s !== sectorName);
    } else {
      updated = [...currentSectors, sectorName];
    }
    setMemberForm({ ...memberForm, sector: updated.join(', ') });
  };

  const removeMemberSector = (sectorName: string) => {
    const currentSectors = memberForm.sector
      ? memberForm.sector.split(',').map((s) => s.trim()).filter(Boolean)
      : [];
    const updated = currentSectors.filter((s) => s !== sectorName);
    setMemberForm({ ...memberForm, sector: updated.join(', ') });
  };

  const handleSubmitMember = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!memberForm.sector.trim()) {
      alert('Por favor seleccione al menos un sector/categoría.');
      return;
    }
    setSubmitting(true);
    try {
                          ...
                          {mem.sector
                            ? mem.sector
                                ...
                                .map((sec) => (
                                  <span
                                    key={sec}
                                    className="inline-block text-[10.5px] font-bold px-2 py-0.5 rounded-md bg-blue-50 text-blue-800 border border-blue-200/60"
                                  >
                                    {sec}
                                  </span>
                                ))
                            : <span className="text-slate-400 italic text-[11px]">Sin sector</span>}
                        </div>
                      </td>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5 relative">
              <div className="flex items-center justify-between">
                <label className="font-bold text-slate-700">Sectores / Categorías *</label>
                <button
                  type="button"
                  onClick={() => {
                    setIsMemberModalOpen(false);
              ...
              {/* Badges de sectores seleccionados */}
              {memberForm.sector ? (
                <div className="flex flex-wrap gap-1.5 mb-1.5">
                  {memberForm.sector
                    ...
                    .map((sec) => (
                      <span
                        key={sec}
                        className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-blue-50 border border-blue-200 text-blue-800 text-[11px] font-bold"
                      >
                        <span>{sec}</span>
                        <button
                          type="button"
                          onClick={() => removeMemberSector(sec)}
                          className="text-blue-500 hover:text-rose-600 rounded p-0.5"
              ...
              <div className="relative">
                <button
                  ...
                  className={`w-full px-3.5 py-2.5 rounded-xl border text-left flex items-center justify-between transition-all bg-white text-xs ${
                    !memberForm.sector
                      ...
                      : 'border-blue-300 text-slate-900 font-semibold bg-blue-50/20'
                  }`}
                >
                  <span className="truncate">
                    {memberForm.sector
                      ? `${memberForm.sector.split(',').filter((s) => s.trim()).length} categoría(s) seleccionada(s) (clic para sumar más)`
                      : 'Seleccione una o más categorías de la lista...'}
                  </span>
                ...
                {/* Dropdown Menu con Buscador y Checkboxes */}
                  ...
                  <div className="absolute left-0 right-0 top-full mt-1.5 bg-white rounded-2xl border border-slate-200 shadow-xl z-50 p-2 space-y-2 max-h-64 flex flex-col animate-in fade-in zoom-in-95 duration-100">
                    ...
                    <div className="overflow-y-auto flex-1 divide-y divide-slate-50">
                      {categories
                        ...
                        .map((c) => {
                          const selectedList = memberForm.sector
                            ? memberForm.sector.split(',').map((s) => s.trim()).filter(Boolean)
                            : [];
                          ...
                          return (
                            <button
                              key={c.id}
                              type="button"
                              onClick={() => toggleMemberSector(c.name)}
                            ...
                            >
                              ...
                              <div className="flex items-center gap-1.5 shrink-0">
                                {isSelected ? (
                                  ...
                                  <span className="text-[10px] text-slate-400 font-normal">Agregar</span>
                                )}
                              </div>
                            </button>
                          );
                    ...
                    <div className="pt-2 border-t border-slate-100 flex items-center justify-between px-1">
                      <span className="text-[10.5px] text-slate-400 font-medium">
                        Puedes asignar 1 o más sectores
                      </span>
                      <button
                        type="button"
