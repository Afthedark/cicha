...
export const Navbar: React.FC = () => {
  ...
  const publicNavLinks = [
    ...
    { name: 'Post Redes sociales', path: '/redes-sociales' },
    ...
    <header className="sticky top-0 z-50 w-full transition-all duration-300">
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-3 group py-1 mr-6 sm:mr-8 md:mr-10 lg:mr-12 shrink-0">
            <img
              src={cichaLogo}
              alt="CICHA - Cámara de Industria y Comercio Heleno Argentina"
              className="h-10 sm:h-11 md:h-12 lg:h-13 w-auto object-contain transition-transform group-hover:scale-105 drop-shadow-sm"
            />
          </Link>
          ...
          <button
            ...
            className="lg:hidden p-2 rounded-xl bg-[#D4EEFC]/50 hover:bg-[#D4EEFC] text-[#005EAF] hover:text-[#00AEEF] border border-[#00AEEF]/40 shadow-sm transition-all"
            aria-label={isOpen ? 'Cerrar menú' : 'Abrir menú'}
          >
            {isOpen ? <X className="w-6 h-6 text-[#00AEEF]" /> : <Menu className="w-6 h-6" />}
          </button>
        </div>

        {/* Mobile Dropdown */}
        {isOpen && (
          <div className="lg:hidden bg-gradient-to-b from-[#0B2545] via-[#0A4988] to-[#071E38] border-t-4 border-[#00AEEF] px-4 pt-4 pb-6 space-y-3 max-h-[85vh] overflow-y-auto shadow-2xl backdrop-blur-xl animate-in fade-in slide-in-from-top-2 duration-200">
            {/* Public Links */}
            <div className="space-y-1">
              <span className="text-[10px] font-bold uppercase tracking-wider text-[#00AEEF] px-3 flex items-center gap-1.5">
                <Globe2 className="w-3 h-3" /> Módulos Institucionales & Públicos
              </span>
                  ...
                  return (
                    <Link
                      ...
                      className={`block px-3.5 py-2.5 rounded-xl text-sm font-medium transition-all ${
                        active
                          ? 'text-white bg-[#00AEEF]/25 font-bold border-l-4 border-[#00AEEF] shadow-sm'
                          : 'text-white/90 hover:text-white hover:bg-white/10'
                      }`}
                    >
                      <div className="flex items-center justify-between">
                        <span>{link.name}</span>
                        <ChevronRight className={`w-4 h-4 ${active ? 'text-[#00AEEF]' : 'text-white/40'}`} />
                      </div>
                    </Link>
                  );
                })}
            ...
            <div className="pt-3 border-t border-white/15 space-y-1">
              <span className="text-[10px] font-bold uppercase tracking-wider text-[#F5A623] px-3 flex items-center gap-1.5">
                <Briefcase className="w-3 h-3" /> Área Exclusiva de Socios
              </span>
              <div className="space-y-1 pt-1">
                {partnerLinks.map((item) => {
                  const Icon = item.icon;
                  return (
                    <Link
                      ...
                      to={item.path}
                      onClick={() => setIsOpen(false)}
                      className="flex items-center justify-between px-3.5 py-2.5 rounded-xl text-xs font-semibold text-[#D4EEFC] hover:text-white hover:bg-white/10 transition-all"
                    >
                      <div className="flex items-center gap-2.5">
                        <Icon className="w-4 h-4 text-[#00AEEF]" />
                        <span>{item.name}</span>
                      </div>
                      <ChevronRight className="w-3.5 h-3.5 text-[#00AEEF]/60" />
                    </Link>
                  );
                })}
              </div>
            ...
            <div className="pt-3 flex items-center justify-between gap-2 border-t border-white/15 notranslate px-2">
              <span className="text-xs font-medium text-[#D4EEFC]">Idioma / Language:</span>
            ...
            <div className="pt-2">
              <Link
                to="/asociarse"
                onClick={() => setIsOpen(false)}
                className="w-full flex items-center justify-center gap-2 px-4 py-3.5 rounded-xl bg-gradient-to-r from-[#00AEEF] to-[#0066CC] hover:from-[#0096ce] hover:to-[#0052cc] text-white font-bold text-sm shadow-lg shadow-[#00AEEF]/30 hover:scale-[1.02] transition-all"
              >
